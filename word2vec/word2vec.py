...
def generate_batch(batch_size, context_window):
  ...
  for i in range(batch_size):
    ...
    labels[i, 0] = buffer_list.pop(context_window)
    batch[i] = buffer_list
...
batch, labels = generate_batch(batch_size=8, context_window=5)
...
with graph.as_default():
  ...
  # Input data.
  train_inputs = tf.placeholder(tf.int32, shape=[batch_size, 2 * context_window])
  ...
  # loss = tf.reduce_mean(
  #   tf.nn.sampled_softmax_loss(nce_weights, nce_biases, tf.reduce_sum(embed,1),
...
#Modifying Dictionary for Compatibility
dictionary2=dictionary
for key in dictionary2.keys():
    dictionary2[key]=(dictionary2[key],0)
  
pickle.dump(final_embeddings,open("word2vec_emb.pickle","wb"))
pickle.dump(dictionary2,open("word2vec_dict.pickle","wb"))
